...
const info = {
    paths: ["/", "/product/", "/impact-report/", "/blog/the-complete-guide-to-coffee-shop-profit-margins-and-how-to-raise-them/"],
...
async function run(i: typeof info) {
    ...
    for (const x of urls) {
        ...
        for (const width of widths) {
            ...
            await crawlSite({ ...defaultOptions, totalPages: 0, urls, output: join(i.output, name, `${width}`), width });
        }

    }
}

run(info).then(() => console.log("finished")).catch(console.error);